...
extension DKMFormatter.Writer {
    ...
    private mutating func _writeCommand(_ command: DKMCommand) throws(DKMFormatter.Error) {
        ...
        case let .filterLine(line):
            ...
            _writeLine("\(line.startBeat) \(line.duration) \(line.filterType.rawValue) \(line.startPitch)"
                       + " \(line.endPitch) \(line.startBandwidth) \(line.endBandwidth)")
        ...
        case let .tempoLine(line):
            _writeSection("Tempo")
            _writeLine("\(line.startBeat) \(line.duration) \(line.startTempo) \(line.endTempo)")
