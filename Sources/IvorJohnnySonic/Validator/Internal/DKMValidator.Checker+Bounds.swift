...
extension DKMValidator.Checker {
    ...
    internal mutating func checkTempoValues(startTempo: Double,
                                            endTempo: Double,
                                            at index: Int) {
        if startTempo <= 0 {
            issues.append(.nonPositiveTempo(commandIndex: index, tempo: startTempo))
        }

        if endTempo <= 0 {
            issues.append(.nonPositiveTempo(commandIndex: index, tempo: endTempo))
        }
    }
