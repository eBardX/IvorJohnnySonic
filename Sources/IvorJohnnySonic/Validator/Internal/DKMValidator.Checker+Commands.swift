...
extension DKMValidator.Checker {
    ...
    internal mutating func checkFilterLine(_ line: DKMFilterLine,
                                           at index: Int) {
        checkSegment(startBeat: line.startBeat, duration: line.duration, at: index)
        checkFinite(line.startPitch, parameter: "startPitch", at: index)
        checkFinite(line.endPitch, parameter: "endPitch", at: index)
        checkFinite(line.startBandwidth, parameter: "startBandwidth", at: index)
        checkFinite(line.endBandwidth, parameter: "endBandwidth", at: index)
    }
    ...
    internal mutating func checkTempoLine(_ line: DKMTempoLine,
                                          at index: Int) {
        checkSegment(startBeat: line.startBeat, duration: line.duration, at: index)
        checkFinite(line.startTempo, parameter: "startTempo", at: index)
        checkFinite(line.endTempo, parameter: "endTempo", at: index)
        checkTempoValues(startTempo: line.startTempo, endTempo: line.endTempo, at: index)
    }
