...
extension DKMCommandTests {
    ...
    func equality() {
        ...
        #expect(DKMCommand.tempoLine(DKMTempoLine(startBeat: 0.0,
                                                  duration: 1.0,
                                                  startTempo: 60.0,
                                                  endTempo: 60.0)) == .tempoLine(DKMTempoLine(startBeat: 0.0,
                                                                                              duration: 1.0,
                                                                                              startTempo: 60.0,
                                                                                              endTempo: 60.0)))
    }
    ...
    func filterLine() {
        ...
        if case let .filterLine(line) = command {
            ...
            #expect(line.startPitch == -550.0)
            #expect(line.endPitch == -550.0)
            #expect(line.startBandwidth == -80.0)
            #expect(line.endBandwidth == -80.0)
        } else {
    ...
    @Test
    func tempoLine() {
        let command = DKMCommand.tempoLine(DKMTempoLine(startBeat: 0.0, duration: 1.0, startTempo: 57.0, endTempo: 57.0))

        if case let .tempoLine(line) = command {
            #expect(line.startBeat == 0.0)
            #expect(line.duration == 1.0)
            #expect(line.startTempo == 57.0)
            #expect(line.endTempo == 57.0)
        } else {
