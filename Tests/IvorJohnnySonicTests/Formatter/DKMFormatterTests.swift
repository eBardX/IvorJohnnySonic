...
extension DKMFormatterTests {
    ...
    func formatCommentBetweenSameCommands() throws {
        let score = DKMScore(commands: [.tempoLine(DKMTempoLine(startBeat: 0.0, duration: 1.0, startTempo: 57.0, endTempo: 57.0)),
                                        .comment(" interleaved"),
                                        .tempoLine(DKMTempoLine(startBeat: 137.0, duration: 9.0, startTempo: 57.0, endTempo: 69.0))])
        let result = try formatScore(score)
    ...
    func formatDifferentSections() throws {
        let score = DKMScore(commands: [.tempoLine(DKMTempoLine(startBeat: 0.0, duration: 1.0, startTempo: 60.0, endTempo: 60.0)),
    ...
    func formatMixedScore() throws {
        let score = DKMScore(commands: [.comment(" Header"),
                                        ...
                                        .tempoLine(DKMTempoLine(startBeat: 0.0, duration: 1.0, startTempo: 120.0, endTempo: 120.0)),
                                        .end])
        let result = try formatScore(score)
